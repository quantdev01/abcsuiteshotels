		...
		{block name='blockcart_shopping_cart'}
			<div class="shopping_cart">
				<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}"
					title="{l s='View my booking cart' mod='blockcart'}" rel="nofollow">
					<!-- <b>{l s='Cart' mod='blockcart'}</b> -->
					<span
						class="badge badge_style ajax_cart_quantity{if $cart_qties == 0} unvisible{/if}">{$total_products_in_cart}</span>
					...
					{if $ajax_allowed && isset($blockcart_top) && !$blockcart_top}
						<span
							class="block_cart_expand{if !isset($colapseExpandStatus) || (isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded')} unvisible{/if}">&nbsp;</span>
						<span
							class="block_cart_collapse{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'collapsed'} unvisible{/if}">&nbsp;</span>
					{/if}
				</a>
					...
					{if !$PS_CATALOG_MODE}
						<div class="cart_block block exclusive">
							<div class="block_content">
								...
								<div
									class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
																...
																{block name='blockcart_shopping_cart_product_name'}
																	<div class="product-name">
																		...
																		<a class="cart_block_product_name"
																			...
																			title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...':true|escape:'html':'UTF-8'}</a>
																	</div>
																{/block}
																...
																{block name='blockcart_shopping_cart_product_total_price'}
																	<div class="cart-info-sec rm_product_info_{$product.id_product}">
																		...
																		<span class="price product_info_data"
																			ttl_prod_price="{$product.bookingData.total_room_type_amount}">
																	...
																	<div class="cart-info-sec rm_product_info_{$product.id_product}">
																		...
																		<span class="quantity-formated">
																			{if $product.booking_product}
																				<span
																					class="quantity product_info_data">{$product.bookingData['total_num_rooms']}</span>
																			{else}
															...
															<span class="remove_link">
																{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
																	<a class="ajax_cart_block_remove_link"
																		href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}"
									)|escape:'html':'UTF-8'}" rel="nofollow"
																				...
																				<tbody>
																					...
																					{foreach from=$product.bookingData['date_diff'] key=data_k1 item=data_v}
																						<tr class="rooms_remove_container">
																							...
																							<td>{convertPrice price=($data_v['amount'] + $data_v['demand_price'])}
																							</td>
																							<td><a class="remove_rooms_from_cart_link" href="#"
																									rm_price="{$data_v['amount']}"
																									id_product="{$product.id_product|intval}"
																									date_from="{$data_v['data_form']}"
																									date_to="{$data_v['data_to']}" num_rooms="{$data_v['num_rm']}"
																									title="{l s='remove this room from my cart' mod='blockcart'}"></a>
																							</td>
																						</tr>
																					{/foreach} *}
																				</tbody>
																	...
																	{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
																		<li name="customization">
																			<div data-id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}"
																				...
																				<a class="ajax_cart_block_remove_link"
																					href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_customization={$id_customization|intval}&token={$static_token}"
											)|escape:'html':'UTF-8'}" rel="nofollow">&nbsp;</a>
																			</div>
									...
									{* {block name='blockcart_shopping_cart_discounts'}
										...
										{if $discounts|@count > 0}
											...
											{if $discounts|@count == 0} unvisible
												...
												{if $discount.value_real > 0}
													...
													{if strlen($discount.code)}
																																																														<a class="delete_voucher"
																																																															href="{$link->getPageLink("$order_process", true)}?deleteDiscount={$discount.id_discount|intval}"
																																																															title="




														{l s='Delete' mod='blockcart'}" rel="nofollow">
									...
									{* {block name='blockcart_shopping_cart_prices'} *}
										...
										{block name='blockcart_shopping_cart_total_convenience_fee'}
											...
											{if isset($total_convenience_fee)}
																																																																																										<div class="cart-prices-line">
																																																																																											<span
																																																																																												class="price cart_block_convenience_fee ajax_cart_convenience_fee">
												...
												{convertPrice price=$total_convenience_fee}</span>
																																																																																											<span class="price">
												...
												{l s='Convenience Fees' mod='blockcart'}</strong>
																																																																																										</div>
									...
									{block name='blockcart_shopping_cart_checkout_action'}
										<p class="cart-buttons">
											<a id="button_order_cart" class="btn btn-default button button-small"
												...
												title="{l s='Check out' mod='blockcart'}" rel="nofollow">
												<span>
						...
						<div class="layer_cart_product col-xs-12 col-md-6">
							...
							{block name='blockcart_layer_cart_product_info'}
								<div class="layer_cart_product_info">
									...
									<div>
										<strong
											class="dark layer_cart_room_txt">{if isset($occupancy_required_for_booking) && $occupancy_required_for_booking}{l s='Room occupancy' mod='blockcart'}{else}{l s='Rooms quantity added' mod='blockcart'}{/if}
											&nbsp;-&nbsp;</strong>
										<strong class="dark layer_cart_product_txt">{l s='Quantity' mod='blockcart'} &nbsp;-&nbsp;</strong>
										<span id="layer_cart_product_quantity"></span>
									</div>
									<div>
										<strong class="dark layer_cart_room_txt">{l s='Room type cost' mod='blockcart'}
											&nbsp;-&nbsp;</strong>
							...
							{block name='blockcart_layer_cart_total_convenience_fee'}
								{if isset($total_convenience_fee)}
									<div class="layer_cart_row">
										...
										<span
											class="price ajax_cart_convenience_fee pull-right">{convertPrice price=$total_convenience_fee}</span>
									</div>
								{/if}
							{/block}
								...
								{block name='blockcart_layer_cart_actions'}
									<div class="button-container">
										<span class="continue btn btn-default button exclusive-medium"
											title="{l s='Continue browsing' mod='blockcart'}">
											<span>
