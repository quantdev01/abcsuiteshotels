							...
							{block name='order_opc_left_column'}
								<div class="col-md-8">
									...
									<div class="accordion" id="oprder-opc-accordion">
										<input type="hidden" name="opc_id_address_delivery" value="{$cart->id_address_delivery}"
											id="opc_id_address_delivery" />
										<input type="hidden" name="opc_id_address_invoice" value="{$cart->id_address_invoice}"
														...
														<div class="card">
															<div class="card-header" id="shopping-cart-summary-head">
																{block name='order_opc_rooms_summary_heading'}
																	<h5 class="accordion-header" data-toggle="collapse"
																		data-target="#collapse-shopping-cart" aria-expanded="true"
																		aria-controls="collapse-shopping-cart">
																		<span>{l s='Rooms & Price Summary'}</span>
																		<i
																			class="icon-angle-left pull-right accordion-left-arrow {if $step->step_is_current}hidden{/if}"></i>
																	</h5>
																{/block}
															</div>
															{if $step->step_is_reachable}
																<div id="collapse-shopping-cart"
																	class="opc-collapse {if !$step->step_is_current}collapse{/if}"
																	aria-labelledby="shopping-cart-head" data-parent="#oprder-opc-accordion">
																	<div class="card-body">
														...
														<div class="card" id="guest-info-block">
															<div class="card-header" id="guest-info-head">
																{block name='order_opc_guest_information_heading'}
																	<h5 class="accordion-header" data-toggle="collapse" data-target="#collapse-guest-info"
																		aria-expanded="true" aria-controls="collapse-guest-info">
															...
															{if $step->step_is_reachable}
																{block name='order_opc_guest_detail_wrapper'}
																	<div id="collapse-guest-info" class="opc-collapse {if !$step->step_is_current}collapse{/if}"
																		aria-labelledby="guest-info-head" data-parent="#oprder-opc-accordion">
																		<div class="card-body">
																			{if $is_logged || $isGuest}
																				{if $is_logged}
																					{block name='order_opc_guest_detail_form'}
																						<form id="customer_guest_detail_form">
																							<p class="checkbox">
																								<input type="checkbox" name="customer_guest_detail"
																									id="customer_guest_detail" value="1"
																									{if $id_customer_guest_detail}checked="checked" {/if} />
																								<label for="customer_guest_detail"
																									id="customer_guest_detail_txt">{l s='Booking for someone else?'}</label>
																							</p>
																							<div id="customer-guest-detail-container"
																								{if !$id_customer_guest_detail}style="display: none;" {/if}>
																								<div class="row">
																									<div class="required clearfix gender-line col-sm-2">
																										<label>{l s='Social title'}</label>
																										<select name="customer_guest_detail_gender"
																											id="customer_guest_detail_gender">
																											{foreach from=$genders key=k item=gender}
																												<option value="{$gender->id_gender}"
																													{if isset($smarty.post.customer_guest_detail_gender) && $smarty.post.customer_guest_detail_gender == $gender->id_gender || (isset($customer_guest_detail) && $customer_guest_detail.id_gender == $gender->id_gender)}
																													selected="selected" {/if}>{$gender->name}</option>
																											{/foreach}
																										</select>
																									</div>
																									<div class="required form-group col-sm-5">
																										<label for="firstname">{l s='First name'} <sup>*</sup></label>
																										<input type="text" class="text form-control validate is_required"
																											id="customer_guest_detail_firstname"
																											name="customer_guest_detail_firstname" data-validate="isName"
																											data-maxSize="32"
																											{if isset($smarty.post.customer_guest_detail_firstname) && $smarty.post.customer_guest_detail_firstname}
																												value="{$smarty.post.customer_guest_detail_firstname}"
																											{elseif isset($customer_guest_detail) && $customer_guest_detail.firstname}
																											value="{$customer_guest_detail.firstname}" {/if} />
																									</div>
																									<div class="required form-group col-sm-5">
																										<label for="lastname">{l s='Last name'} <sup>*</sup></label>
																										<input type="text" class="form-control validate is_required"
																											id="customer_guest_detail_lastname"
																											name="customer_guest_detail_lastname" data-validate="isName"
																											data-maxSize="32"
																											{if isset($smarty.post.customer_guest_detail_lastname) && $smarty.post.customer_guest_detail_lastname}
																												value="{$smarty.post.customer_guest_detail_lastname}"
																											{elseif isset($customer_guest_detail) && $customer_guest_detail.lastname}
																											value="{$customer_guest_detail.lastname}" {/if} />
																									</div>
																								</div>
																								<div class="row">
																									<div class="required text form-group col-sm-6">
																										<label for="email">{l s='Email'} <sup>*</sup></label>
																										<input type="email" class="text form-control validate is_required"
																											id="customer_guest_detail_email"
																											name="customer_guest_detail_email" data-validate="isEmail"
																											data-maxSize="128"
																											{if isset($smarty.post.customer_guest_detail_email) && $smarty.post.customer_guest_detail_email}
																												value="{$smarty.post.customer_guest_detail_email}"
																											{elseif isset($customer_guest_detail) && $customer_guest_detail.email}
																											value="{$customer_guest_detail.email}" {/if} />
																									</div>
																								</div>
																								<div class="row">
																									<div
																										class="{if isset($one_phone_at_least) && $one_phone_at_least}required {/if}form-group col-sm-6">
																										<label
																											for="phone_mobile">{l s='Mobile phone'}{if isset($one_phone_at_least) && $one_phone_at_least}
																											<sup>**</sup>{/if}</label>
																										<input type="text" class="text form-control validate is_required"
																											name="customer_guest_detail_phone"
																											id="customer_guest_detail_phone" data-validate="isPhoneNumber"
																											data-maxSize="32"
																											{if isset($smarty.post.customer_guest_detail_phone) && $smarty.post.customer_guest_detail_phone}
																												value="{$smarty.post.customer_guest_detail_phone}"
																											{elseif isset($customer_guest_detail) && $customer_guest_detail.phone}
																											value="{$customer_guest_detail.phone}" {/if} />
																									</div>
																								</div>
																							</div>
																						</form>
																				...
																				{block name='order_opc_guest_detail'}
																					<div id="checkout-guest-info-block"
																						{if $id_customer_guest_detail}style="display: none;" {/if}>
																						<div class="row margin-btm-10">
																							<div class="col-sm-3 col-xs-5 info-head">{l s='Name'}</div>
																							<div class="col-sm-9 col-xs-7 info-value">
																								{if $isGuest}
																									{$guestInformations['customer_firstname']|escape:'html':'UTF-8'}
																									{$guestInformations['customer_lastname']|escape:'html':'UTF-8'}
																								{else}
																						...
																						<div class="row margin-btm-10">
																							...
																							<div class="col-sm-9 col-xs-7 info-value">
																								{$guestInformations['email']|escape:'html':'UTF-8'}</div>
																						</div>
																						...
																						<div class="row">
																							<div class="col-sm-12 proceed_btn_block">
																								{* <a class="btn btn-default button button-medium pull-right"
																									href="{$link->getPageLink('order-opc', null, null, ['proceed_to_payment' => 1])}"
																								...
																								<form
																									action="{$link->getModuleLink('bankwire', 'validation', [], true)|escape:'html':'UTF-8'}"
																									method="post">
																									...
																									<p class="cart_navigation clearfix" id="cart_navigation">
																										...
																										{if !$restrict_order}
																											...
																											<button class="btn pull-right button button-medium" type="submit">
																												<span>{l s='Proceed' mod='bankwire'}</span>
																											</button>
																											<a href="https://wa.me/243813616956?text=Bonjour"></a>
																										{/if}
																									</p>
																								</form>
																								{if $isGuest}
																									<a class="btn btn-default btn-edit-guest-info pull-right" href="#"
																										rel="nofollow">
																										<span>
																											{l s='Edit'}
																										</span>
														...
														<div class="card">
															<div class="card-header" id="order-payment-head">
																{block name='order_opc_payment_heading'}
																	<h5 class="accordion-header" data-toggle="collapse"
																		data-target="#collapse-order-payment" aria-expanded="true"
																		aria-controls="collapse-order-payment">
															...
															{if $step->step_is_reachable}
																<div id="collapse-order-payment"
																	class="opc-collapse {if !$step->step_is_current}collapse{/if}"
																	aria-labelledby="order-payment-head" data-parent="#oprder-opc-accordion">
																	<div class="card-body">
																		<!-- Payment -->
									...
									{block name='order_opc_cart_total_detail'}
										...
										<div class="col-sm-12 card cart_total_detail_block">
											...
											<p {if !isset($is_advance_payment) || !$is_advance_payment}class="cart_final_total_block" {/if}>
												...
												{/block}
												<span
													class="cart_total_values {if isset($is_advance_payment) && $is_advance_payment} strong{/if}">
													{if $use_taxes}
										...
										{if $voucherAllowed}
											...
											<div class="col-sm-12 card cart_voucher_detail_block">
												...
												{if sizeof($discounts)}
													<div class="row">
														{foreach $discounts as $discount}
															<div class="col-sm-12 margin-btm-10 cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}"
																...
																<span class="cart_discount_name">
																	...
																	{if strlen($discount.code)}
																		<a href="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}?deleteDiscount={$discount.id_discount}"
																			class="price_discount_delete pull-right" title="{l s='Delete'}">
												...
												<div class="row margin-btm-20">
													...
													<form
														action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}"
														...
														<div class="col-sm-4 col-xs-12 col-md-12 col-lg-4 submit_discount_div">
															<button type="submit" name="submitAddDiscount"
																class="opc-button-small opc-btn-primary">
																<span>{l s='Apply'}</span>
															</button>
														</div>
													</form>
															...
															<div class="col-xs-12">
																<p class="avail_voucher_name">
																	<span class="voucher_name"
																		data-code="{$voucher.code|escape:'html':'UTF-8'}">{$voucher.code|escape:'html':'UTF-8'}
																		-</span>&nbsp;{$voucher.name}
																</p>
						...
						{block name='order_opc_js_vars'}
							{strip}
								...
								{capture}<a class="button button-small btn btn-default" href="{$smarty.capture.addressUrlAdd}"
										title="{l s='Update' js=1}"><span>{l s='Update' js=1}<i
											class="icon-chevron-right right"></i></span></a>{/capture}
